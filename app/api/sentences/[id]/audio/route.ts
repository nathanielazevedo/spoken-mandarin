...
export async function POST(
  ...
  try {
    ...
    const sentence = await prisma.sentence.findUnique({
      where: { id: sentenceId },
      select: { id: true, lessonId: true, pinyin: true },
    });
    ...
    if (!sentence.pinyin) {
      return NextResponse.json({ error: 'Sentence is missing pinyin text' }, { status: 400 });
    }

    const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
    const lessonFolder = sentence.lessonId || 'shared';
